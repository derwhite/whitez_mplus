...
class RaiderIO:
	...
	def __init__(self, Rio_APIKEY, proxy=""):
		self.proxy = proxy
		if Rio_APIKEY != "":
			self.Rio_APIKEY = f'access_key={Rio_APIKEY}&'
		else:
			self.Rio_APIKEY = ""
	...
	def extract_player_ids(self, players) -> None:
		...
		pull_urls = [f'https://raider.io/api/v1/mythic-plus/run-details?{self.Rio_APIKEY}season={season_slug}&id={run_id}' for run_id in run_ids]
	...
	def append_api_requests(self, players_list, season):
		for player in players_list:
			...
			fields2 = ','.join(fields)
			url = f'https://raider.io/api/v1/characters/profile?{self.Rio_APIKEY}region=eu&realm={realm}&name={name}&fields={fields2}'
			player['url'] = url


	def pull(self, urls):    #sometimes gets stuck if Proxy does not response -.-
		with requests.Session() as s:
			...
			if self.proxy != '':
				with open('proxykey') as f:
					key = f.readlines()
					proxies = {
					'http': f'http://{key[0].strip()}@{self.proxy}:8080',
					'https': f'http://{key[0].strip()}@{self.proxy}:8080'
					}
	...
	def get_instances(self, expansion_id, season):
		tmp = self.pull([f'https://raider.io/api/v1/mythic-plus/static-data?{self.Rio_APIKEY}expansion_id={expansion_id}'])
		instances = []
		season_name = ""
	...
	def get_score_colors(self):
		response_list = self.pull([f'https://raider.io/api/v1/mythic-plus/score-tiers?{self.Rio_APIKEY}season=current'])
	...
	def get_current_season(self, expasion_id) -> tuple[str, datetime]:
		down = self.pull([f'https://raider.io/api/v1/mythic-plus/static-data?{self.Rio_APIKEY}expansion_id={expasion_id}'])[0]
