...
class RaiderIO:
    ...
    def get_run_details(self, players):
        ...
        for url, r in zip(urls, runs_response):
            if r.ok and is_json(r.text):
                # add tank first
                get_spieler = [
                    f"[{int(spieler['items']['item_level_equipped'])}] {spieler['character']['name']}"
                    for spieler in r.json()["roster"]
                    if spieler["character"]["spec"]["role"] == "tank"
                ]
                # add healer 2nd
                get_spieler.extend(
                    [
                        f"[{int(spieler['items']['item_level_equipped'])}] {spieler['character']['name']}"
                        for spieler in r.json()["roster"]
                        if spieler["character"]["spec"]["role"] == "healer"
                    ]
                )
                # add dps last
                get_spieler.extend(
    ...
    def get_instances(self, expansion_id, season):
        tmp = self.pull(
            [f"https://raider.io/api/v1/mythic-plus/static-data?{self.Rio_APIKEY}expansion_id={expansion_id}"]
        )
        instances = []
        season_name = ""
    ...
    def get_score_colors(self):
        response_list = self.pull(
            [f"https://raider.io/api/v1/mythic-plus/score-tiers?{self.Rio_APIKEY}season=current"]
        )
...
def get_color(score_tier, points, max=0, dungeons_count=0):
    color = "white"
    if any([x["score"] == None for x in score_tier]):
        return "white"
    if max > 0:
