    ...
    for p in players:
        ...
        for i in range(0, 8):
            ...
            if len(p._data[weekly]) > i:
                ...
                str_html += f'<td title="Points: {p._data[weekly][i]["score"]}\n{run_time}\n{runs_dict[p._data[weekly][i]["keystone_run_id"]]}"><span style="color:{color}">{p._data[weekly][i]["short_name"]} (<a href="{p._data[weekly][i]["url"]}" style="color:{rio.get_color(colors, p._data[weekly][i]["score"] * (len(inis)*2), high, len(inis))}">{p._data[weekly][i]["mythic_level"]}{stern}</a>)</span></td>\n'
            else:
