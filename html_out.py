...
WREWARD = [0, 0, 305, 305, 308, 308, 311, 315, 315, 315, 318]  # Season 2 Midnight
...
def gen_score_table(players, inis, colors):
    ...
    for count, x in enumerate(inis):
        ...
        str_html += (
            f"<th onclick=\"sortTable({count}, 'td_dungeon', '{table_id}')\" "
    ...
    for p in players:
        ...
        for ini in inis:
            ...
            str_html += (
                f'<td class="td_dungeon" title="{ini_score}">'
...
def gen_weekly(players, inis, colors, weekly, runs_dict):
    ...
    str_html += f"<th onclick=\"sortTable(0, 'td_ilvl', '{table_id}')\" class=\"ilvl\">ilvl</th>"
    str_html += (
        f"<th onclick=\"sortTable(0, 'td_twenty', '{table_id}')\" class=\"twenty\">+{KEY_LEVEL_TO_GET_MYTH_GEAR}</th>"
    )
    ...
    for p in players:
        ...
        for i in range(0, 8):
            ...
            if len(p._data[weekly]) > i:
                ...
                str_html += f'<td title="Points: {p._data[weekly][i]["score"]}\n{run_time}\n{runs_dict[p._data[weekly][i]["keystone_run_id"]]}"><span style="color:{color}">{p._data[weekly][i]["short_name"]} (<a href="{p._data[weekly][i]["keystone_run_id"]}" style="color:{rio.get_color(colors, p._data[weekly][i]["score"] * (len(inis)*2), high, len(inis))}">{p._data[weekly][i]["mythic_level"]}{stern}</a>)</span></td>\n'
            else:
...
def gen_affixes_html(affixes):
    affixes_html_table = (
        '<table><tr><th class="tbl_affixe" spancol=5 style="font-size: 20px;">Affixes:</th></tr>\n{rows}</table>'
    )

    tweek_affixes_html = []
    for a in affixes["this_week"]:
        affix_html = (
            f'<td class="tbl_affixe"><a class="icontiny" '
