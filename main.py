...
def parse_config_file(config_file_path):
    ...
    min_ilvl = get_int_setting("min_ilvl", 250)
...
def cli():
    parser = ArgumentParser(
        description="Pulls information about M+ runs from a custom list of wow mains via Raider.io "
        "and Battle.net API.",
...
def main():
    ...
    inactive_filtered_players = list(
        filter(lambda player: player.days_since_last_update() < settings["max_inactive_days"], score_sorted_players)
    )
    ...
    general_players = (
        inactive_filtered_players  # show all active chars in general tab regardless of their score or ilvl
    )
    # ---------------------
    ...
    tables.update(
        {"main_weekly": html_out.gen_weekly(mains, inis, scolors, "mythic_plus_weekly_highest_level_runs", runs_dict)}
    )
    tables.update(
        {
            "main_pweek": html_out.gen_weekly(
                mains, inis, scolors, "mythic_plus_previous_weekly_highest_level_runs", runs_dict
            )
        }
    )
    # Alts
    ...
    tables.update(
        {"alts_weekly": html_out.gen_weekly(alts, inis, scolors, "mythic_plus_weekly_highest_level_runs", runs_dict)}
    )
    tables.update(
        {
            "alts_pweek": html_out.gen_weekly(
                alts, inis, scolors, "mythic_plus_previous_weekly_highest_level_runs", runs_dict
            )
        }
    )
